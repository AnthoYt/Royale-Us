...
using System.Text;
using Royale Us.Utilities;
using static Royale Us;
using static Royale Us.CustomOption;
using Reactor.Utilities.Extensions;
using AmongUs.GameOptions;
using TMPro;
using Royale Us.Modules;
using AmongUs.Data;
...
namespace Royale Us; {
    public class CustomOption {
        ...
        public CustomOption(int id, CustomOptionType type, string name,  System.Object[] selections, System.Object defaultValue, CustomOption parent, bool isHeader, string format, string heading = "") {
            ...
            selection = 0;
            if (id != 0) {
                entry = Royale UsPlugin.Instance.Config.Bind($"Preset{preset}", id.ToString(), defaultSelection);
        ...
        public static void switchPreset(int newPreset) {
            ...
            vanillaSettings = Royale UsPlugin.Instance.Config.Bind($"Preset{preset}", "GameOptions", "");
            loadVanillaOptions();
            foreach (CustomOption option in CustomOption.options) {
                if (option.id == 0) continue;

                option.entry = Royale UsPlugin.Instance.Config.Bind($"Preset{preset}", option.id.ToString(), option.defaultSelection);
        ...
        public static bool loadVanillaOptions() {
            ...
            {
             Royale UsPlugin.Logger.LogMessage("tried to paste old settings, not doing this!");
                return false;
            }
        ...
        public static int deserializeOptions(byte[] inputValues) {
            ...
            while (reader.BaseStream.Position < inputValues.Length) {
                try {
                 ...
                 Royale UsPlugin.Logger.LogWarning($"id:{lastId}:{e}: while deserializing - tried to paste invalid settings!");
                    errors++;
                }
            }
        ...
        public static void copyToClipboard() {
            GUIUtility.systemCopyBuffer = $" Royale UsPlugin.VersionString}!{Convert.ToBase64String(serializeOptions())}!{vanillaSettings.Value}";
        }
        ...
        public static int pasteFromClipboard() {
            ...
            try {
                ...
                ShareOptionSelections();
                if  Royale UsPlugin.Version > versionInfo && versionInfo < Version.Parse("1.2.7"))
                {
             ...
             Royale UsPlugin.Logger.LogWarning($"{e}: tried to paste invalid settings!\n{allSettings}");
    ...
    class LobbyViewSettingsPatch
    {
        ...
        public static void drawTab(LobbyViewSettingsPane __instance, CustomOptionType optionType)
        {
            ...
            foreach (var option in relevantOptions)
            {
                if (option.isHeader && (int)optionType != 99 || (int)optionType == 99 && curType != option.type)
                {
                    ...
                    if ((int)optionType == 99)
                        categoryHeaderMasked.Title.text = new Dictionary<CustomOptionType, string>() { { CustomOptionType.Impostor, ModTranslation.getString("impostorRoles") },
                ...
                if (option == CustomOptionHolder.crewmateRolesCountMax || option == CustomOptionHolder.impostorRolesCountMax || option == CustomOptionHolder.modifiersCountMax || option == CustomOptionHolder.crewmateRolesFill)
                    continue;
                ...
                if (option.isHeader && (int)optionType != 99 && option.heading == "" && (| option.type == CustomOptionType.Crewmate || option.type == CustomOptionType.Impostor || option.type == CustomOptionType.Modifier))
                {
        ...
        public static void createSettingTabs(LobbyViewSettingsPane __instance)
        {
            ...
            int next = 3;
            if (| TORMapOptions.gameMode == CustomGamemodes.Classic || TORMapOptions.gameMode == CustomGamemodes.FreePlay)
            {
    ...
    class CreateGameOptionsTORBehaviour : MonoBehaviour
    {
        ...
        void OnEnable()
        {
            ...
            //Les options pour 4 personnes ou plus ne peuvent être utilisées que sur des serveurs personnalisés
    ...
    class CreateGameOptionsShowPatch
    {
        ...
        {
            // Étendre la longueur du tableau de divers paramètres à 24 personnes
           ...
           // Le mode de jeu est fixé à normal
    ...
    class GameOptionsMenuStartPatch
    {
        ...
        public static void Postfix(GameSettingMenu __instance)
        {
            ...
            if (GameOptionsManager.Instance.currentGameOptions.GameMode == GameModes.Cache-Cache) return;
            ...
            bgrenderer.sprite = Helpers.loadSpriteFromResources( Royale Us.Resources.CopyPasteBG.png", 175f);
            ...
            copyButtonRenderer.sprite = Helpers.loadSpriteFromResources( Royale Us.Resources.Copy.png", 100f);
            copyButton.transform.GetChild(1).transform.localPosition = Vector3.zero;
            copyButtonActiveRenderer.sprite = Helpers.loadSpriteFromResources( Royale Us.Resources.CopyActive.png", 100f);
            ...
            pasteButtonRenderer.sprite = Helpers.loadSpriteFromResources( Royale Us.Resources.Paste.png", 100f);
            pasteButtonActiveRenderer.sprite = Helpers.loadSpriteFromResources( Royale Us.Resources.PasteActive.png", 100f);
        ...
        private static void createSettings(GameOptionsMenu menu, List<CustomOption> options)
        {
            ...
            foreach (CustomOption option in options)
            {
                ...
                if (option.isHeader && option.heading == "" && (|| option.type == CustomOptionType.Crewmate || option.type == CustomOptionType.Impostor || option.type == CustomOptionType.Modifier))
                {
        ...
        private static void createSettingTabs(GameSettingMenu __instance)
        {
            ...
            else if (TORMapOptions.gameMode == CustomGamemodes.Cache-Cache)
            {
                // create Main HNS settings
                createCustomButton(__instance, next++, "Cache-CacheMain", ModTranslation.getString("Cache-CacheMain"));
                createGameOptionsMenu(__instance, CustomOptionType.Cache-CacheMain, "Cache-CacheMain");
                // create HNS Role settings
                createCustomButton(__instance, next++, "Cache-CacheRoles", ModTranslation.getString("Cache-CacheRoles"));
                createGameOptionsMenu(__instance, CustomOptionType.Cache-CacheRoles, "Cache-CacheRoles");
            }
        }
    }
    ...
    class GameOptionsDataPatch
    {
        ...
        private static string buildRoleOptions() {
            ...
            return impRoles + crewRoles + modifiers;
        }
        ...
        private static string buildOptionsOfType(CustomOption.CustomOptionType type, bool headerOnly) {
            ...
            if (TORMapOptions.gameMode == CustomGamemodes.Guesser) {
                if (type == CustomOption.CustomOptionType.General)
                    options = CustomOption.options.Where(o => o.type == CustomOption.CustomOptionType.Guesser);
                ...
                options = options.Where(x => !(x.type == CustomOptionHolder.crewmateRolesFill || x.id == 7007));
            else if (TORMapOptions.gameMode == CustomGamemodes.Cache-Cache)
                options = options.Where(x => (x.type == CustomOption.CustomOptionType.Cache-CacheMain || x.type == CustomOption.CustomOptionType.Cache-CacheRoles));
            if (TORMapOptions.gameMode != CustomGamemodes.FreePlay)
            ...
            foreach (CustomOption option in options) {
                if (TORMapOptions.gameMode == CustomGamemodes.Cache-Cache && option.type != CustomOptionType.Cache-CacheMain && option.type != CustomOptionType.Cache-CacheRoles) continue;
                if (option.parent != null) {
                    ...
                    if (option == CustomOptionHolder.crewmateRolesCountMin) {
                        ...
                        var optionValue = (min == max) ? $"{max}" : $"{min} - {max}";
                        sb.AppendLine($"{optionName}: {optionValue}");
                    } else if ((option == CustomOptionHolder.crewmateRolesCountMax) || (option == CustomOptionHolder.impostorRolesCountMax) || option == CustomOptionHolder.modifiersCountMax) {
                        continue;
                    } else {
        ...
        public static string buildAllOptions(string vanillaSettings = "", bool hideExtras = false) {
            ...
            int counter = Royale UsPlugin.optionsPage;
            ...
            int maxPage = 7;
            if (TORMapOptions.gameMode == CustomGamemodes.Cache-Cache) {
                if  Royale UsPlugin.optionsPage > 1) Royale UsPlugin.optionsPage = 0;
                maxPage = 2;
                switch (counter) {
                    case 0:
                        hudString += ModTranslation.getString("Cache-CachePage1") + buildOptionsOfType(CustomOption.CustomOptionType.Cache-CacheMain, false);
                        break;
                    case 1:
                        hudString += ModTranslation.getString("Cache-CachePage2") + buildOptionsOfType(CustomOption.CustomOptionType.Cache-CacheRoles, false);
                        break;
                }
            } else {
                switch (counter) {
        ...
        {
            if (GameOptionsManager.Instance.currentGameOptions.GameMode == AmongUs.GameOptions.GameModes.Cache-Cache) return; // Allow Vanilla Hide N Seek
    ...
    public static class GameOptionsNextPagePatch
    {
        ...
        {
            int page = Royale UsPlugin.optionsPage;
            if (Input.GetKeyDown(KeyCode.Tab)) {
             Royale UsPlugin.optionsPage =  Royale UsPlugin.optionsPage + 1) % 7;
            }
            if (Input.GetKeyDown(KeyCode.Alpha1) || Input.GetKeyDown(KeyCode.Keypad1)) {
             Royale UsPlugin.optionsPage = 0;
            }
            if (Input.GetKeyDown(KeyCode.Alpha2) || Input.GetKeyDown(KeyCode.Keypad2)) {
             Royale UsPlugin.optionsPage = 1;
            }
            if (Input.GetKeyDown(KeyCode.Alpha3) || Input.GetKeyDown(KeyCode.Keypad3)) {
             Royale UsPlugin.optionsPage = 2;
            }
            if (Input.GetKeyDown(KeyCode.Alpha4) || Input.GetKeyDown(KeyCode.Keypad4)) {
             Royale UsPlugin.optionsPage = 3;
            }
            if (Input.GetKeyDown(KeyCode.Alpha5) || Input.GetKeyDown(KeyCode.Keypad5)) {
             Royale UsPlugin.optionsPage = 4;
            }
            if (Input.GetKeyDown(KeyCode.Alpha6) || Input.GetKeyDown(KeyCode.Keypad6)) {
             Royale UsPlugin.optionsPage = 5;
            }
            if (Input.GetKeyDown(KeyCode.Alpha7) || Input.GetKeyDown(KeyCode.Keypad7)) {
             Royale UsPlugin.optionsPage = 6;
            }
    ...
    public class HudManagerUpdate {
        ...
        public static void Prefix2(HudManager __instance) {
            ...
            for (int i = 0; i < blockCount; i++) {
             Royale UsPlugin.Logger.LogMessage(blockCount);
        ...
        public static void Postfix(HudManager __instance) {
            ...
            if (!toggleSettingsButton || !toggleSettingsButtonObject) {
                ...
                renderer.sprite = Helpers.loadSpriteFromResources( Royale Us.Resources.Settings_Button.png", 100f);
                rendererActive.sprite = Helpers.loadSpriteFromResources( Royale Us.Resources.Settings_ButtonActive.png", 100);
            ...
            toggleSettingsButtonObject.SetActive(__instance.MapButton.gameObject.active && !(MapBehaviour.Instance && MapBehaviour.Instance.IsOpen) && GameOptionsManager.Instance.currentGameOptions.GameMode != GameModes.Cache-Cache);
            ...
            if (!toggleZoomButton || !toggleZoomButtonObject)
            {
                ...
                tZrenderer.sprite = Helpers.loadSpriteFromResources( Royale Us.Resources.Minus_Button.png", 100f);
                tZArenderer.sprite = Helpers.loadSpriteFromResources( Royale Us.Resources.Minus_ButtonActive.png", 100);
